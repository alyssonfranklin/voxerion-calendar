...
function getFallbackLabels() {
  return {
    welcome_title: 'Welcome to Kantor',
    ...
    contact_admin: 'Please contact your administrator to get access to Kantor.',
...
function makeOpenAIRequest(prompt) {
  try {
    ...
    // Get Kantor's API key from Script Properties
...
function createUnregisteredUserCard(labels, userEmail) {
  ...
  section.addWidget(
    CardService.newImage()
      .setImageUrl('https://drive.google.com/uc?export=view&id=1M142uYRVzXdYZhADU22oKMQHX0KGnuGb')
      .setAltText('Kantor Logo')
  );
  ...
  section.addWidget(
    CardService.newDecoratedText()
      .setText(`The email ${userEmail} is not registered with Kantor.`)
      .setWrapText(true)
  );
