...
const AUTH_CONFIG = {
  ...
  DB_SPREADSHEET_ID: '1BSbZyBdsV_x4sAhkrayqxHKrZKEof7Fojm6vbi4IKc4', // Replace with your database spreadsheet ID
  MAX_RETRIES: 7,
  RETRY_DELAY_MS: 1000,
  TOKEN_EXPIRY_MINUTES: 15,
  LOGO_CACHE_KEY: 'VOXERION_LOGO_CACHE',
  LOGO_URL: 'https://drive.google.com/uc?export=view&id=1-UmNKwwn-HUrv-a9tDKUu_lOYY50BiUI',
  // Base64 encoded small SVG for immediate display while actual logo loads
  LOGO_PLACEHOLDER: 'data:image/svg+xml;base64,'
};

/**
 * Retrieves localized UI labels from a CSV file stored in Google Drive.
 * Falls back to default labels if the file cannot be accessed.
 * 
...
function getLogoUrl() {
  try {
    ...
    // If the Drive URL should be cached, uncomment this section
    // and replace return statements below with "return logoUrl"
    /*
    // We'll use data URL if possible for maximum performance
    try {
      const logoResponse = UrlFetchApp.fetch(AUTH_CONFIG.LOGO_URL, {
        'method': 'get',
      ...
      if (logoResponse.getResponseCode() === 200) {
        // Get the image bytes
        const imageBlob = logoResponse.getBlob();
        // Convert to base64
        const encodedImage = Utilities.base64Encode(imageBlob.getBytes());
        // Create a data URL
        const mimeType = imageBlob.getContentType();
        const logoUrl = `data:${mimeType};base64,${encodedImage}`;
        
        // Store in script cache for 6 hours
        scriptCache.put(AUTH_CONFIG.LOGO_CACHE_KEY, logoUrl, 21600);
        return logoUrl;
      }
    } catch (e) {
  ...
  try {
    ...
    section.addWidget(
      CardService.newTextParagraph()
        .setText(`Access Required: The email ${userEmail} is not registered with Voxerion.`)
    );
  }
      ...
      CardService.newButtonSet()
        .addButton(CardService.newTextButton()
          ...
          .setOpenLink(CardService.newOpenLink()
            .setUrl('mailto:support@kantorapp.com')))
        .addButton(CardService.newTextButton()
          .setText('Visit kantorapp.com')
          .setOpenLink(CardService.newOpenLink()
            .setUrl('https://kantorapp.com')
